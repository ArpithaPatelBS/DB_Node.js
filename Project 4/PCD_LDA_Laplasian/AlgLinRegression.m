...
function [] =  AlgLinRegression(z, filePathInput)
   
    inputFile = load(filePathInput);
    p=input('Please enter dimension\n');
    %InputData = KFOLD_PCA(z, inputFile,p);
    %InputData = KFOLD_LDA(z, inputFile,p);
    InputData = KFOLD_Laplace(z, inputFile,p);
    ...
    for m = 1 : z
        ...
        Xtest = double(XYtest(: , 2:size(XYtest, 2)))';
            ...
            %for i = 1 : 26
                ...
                k = i -1;
                for j = 1 + k*size(Xtrain,2)/40 : i*size(Xtrain,2)/40
                %for j = 1 + k*32 : i*32
                    Ytrain(i , j) = 1;
                end
